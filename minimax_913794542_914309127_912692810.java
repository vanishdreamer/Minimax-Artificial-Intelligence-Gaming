import java.util.*;
public class minimax_913794542_914309127_912692810 extends AIModule
{
    private int[] moves;
    @Override
    ...
    public int miniMax(final GameStateModule state, int depth, int player){
        ...
        for (i = 0; i < state.getWidth(); i++){
            if (state.canMakeMove(i)){
                state.makeMove(i);
                moves[i] = minV(state, depth + 1, player);
                state.unMakeMove();
                if (moves[i] > value) {
                    value = moves[i];
                    position = i;
    ...
    public int minV(final GameStateModule state, int depth, int player) {
        ...
        for (i = 0; i < state.getWidth(); i++){
            if (state.canMakeMove(i)){
                state.makeMove(i);
                moves[i] = maxV(state, depth + 1, player);
                state.unMakeMove();
                flag = 1;
                if (moves[i] < value) {
                    value = moves[i];
    ...
    public int maxV(final GameStateModule state, int depth, int player){
